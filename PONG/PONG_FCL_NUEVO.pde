...
void iniciovariables() {
  ...
  velxbola=random(1, 8); // velocidad x aleatoria de la bola
  velybola=random(1, 8); // velocidad y de la bola
  altopala=15; //alto de la barra
  anchopala=100; // ancho de la barra
  posypala=450; // posicion en y de la barra
  posxpala=0; // posicion x de la barra
}
...
void moverpala() {
  ...
  posxpala=mouseX; // posicion de la pala en X
}  
...
void rebotes() {
 ...
 //REBOTE CON PALA
  if ((posybola+radio>=posypala) && (posxpala-anchopala/2<posxbola) && (posxpala+anchopala/2>posxbola)) {
    velybola=velybola*-1;
  }
